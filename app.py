...
# ---------------------------------------------------------
# INTERFACCIA BANCONE (Tutto integrato qui)
# ---------------------------------------------------------
if ruolo == "banco":
    st_autorefresh(interval=5000, key="banco_refresh")
    st.markdown("<h2 style='text-align: center;'>👨‍🍳 GESTIONE BANCONE & CASSA</h2>", unsafe_allow_html=True)
    ...
    if not ordini_attuali:
        st.info("In attesa di nuovi ordini dai tavoli...")
    else:
        ...
        for idx, t in enumerate(tavoli_attivi):
            with cols[idx % 3]:
                with st.container(border=True):
                    ...
                    # IL TASTO CASSA: PAGATO E CHIUDI TAVOLO
...
# ---------------------------------------------------------
# INTERFACCIA CLIENTE (Resta al link normale)
# ---------------------------------------------------------
else:
    st.markdown("<h2 style='text-align: center;'>☕ BAR PAGANO - ORDINA ORA</h2>", unsafe_allow_html=True)
    ...
    if st.session_state.tavolo is None:
        ...
        if not menu_df.empty:
            ...
            for _, r in menu_df[menu_df['categoria'] == cat_scelta].iterrows():
        ...
        if st.session_state.carrello:
            ...
            if st.button(f"🚀 INVIA ORDINE AL BANCONE (€{tot:.2f})", type="primary", use_container_width=True):
