...
# ---------------------------------------------------------
# 1. GESTIONE BANCONE (URL: .../?ruolo=banco)
# ---------------------------------------------------------
if ruolo == "banco":
    st_autorefresh(interval=5000, key="refresh_banco")
    st.title("👨‍🍳 Console di Gestione")
            ...
            for idx, t in enumerate(tavoli_attivi):
                with cols[idx % 3]:
                    with st.container(border=True):
                        ...
                        # Tasto PAGATO E CHIUDI integrato sull'ordine
...
# ---------------------------------------------------------
# 2. INTERFACCIA CLIENTE (URL: .../)
# ---------------------------------------------------------
else:
    st.title("☕ Ordina al Bar Pagano")
    ...
    if st.session_state.tavolo is None:
        st.subheader("Seleziona il tuo numero di tavolo:")
    ...
    # MENU E ORDINAZIONE
        ...
        if menu_df.empty:
            ...
            items_menu = menu_df[menu_df['categoria'] == scelta]
            for _, row in items_menu.iterrows():
